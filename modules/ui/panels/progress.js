...
const findNearest = (list, ourLat, ourLng) => {
    ...
    for (const item of list) {
        const [thisLng, thisLat] = item.geo;
...
export function uiPanelProgress(context) {
    function getNext() {
    ...
    function redraw(selection) {
        ...
        if (length) {
            ...
            selection
                .append('span')
                .html('🥰 Done! You\'ve imported/updated all features in ' + window._mostRecentDsId);
        }
    ...
    panel.key = t('info_panels.progress.key');
